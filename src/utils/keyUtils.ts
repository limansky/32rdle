...
export function wordKeyState(
  statuses: Array<Array<[string, LetterState]>>,
): Map<string, KeyState> {
  ...
  for (const ws of statuses) {
    ws.forEach(([c, s]) => {
      const newState =
        s == LetterState.Guess
          ? KeyState.Guess
          : s == LetterState.WrongPosition
            ? KeyState.WrongPosition
            : KeyState.Absent;
...
export function globalKeyState(
  answer: string[],
  words: string[],
  states: BoardState[],
): Map<string, KeyState> {
  ...
  const allLetters = new Set(
    answer.flatMap((x, i) => (states[i] !== BoardState.Solved ? [...x] : [])),
  );
  return new Map(
    ALPHABET.map((l) => [
      l,
