...
export function Keyboard({
  ...
  function createKey(s: string) {
    ...
    switch (s) {
      case "bksp":
        return Key("⌫", onBackspace, KeyState.Special);
      case "enter":
        return Key("⏎", onEnter, KeyState.Special);
      default:
        return Key(s, onLetter, state);
    }
  }

  const rows = layout.map((keys, idx) => (
    <div className={"keyboard-row-" + (idx + 1)} key={"kbdRow" + idx}>
      {keys.map((k) => createKey(k))}
    </div>
  ));
